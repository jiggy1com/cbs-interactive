...
/* old code
var Blog = function(el, blogID){

	var self 		= this;
	this.blogID 	= blogID; //10861780'; //897802273693999289
	this.element 	= el;
	this.APIKey 	= 'AIzaSyBMmDSKIlQWWOz4Ncn2zZWW9YTAR-C8a9M';
	this.loading 			= false;
	this.loadingComments 	= false;
	...
	this.showBlogSpinner = function(){
		var spinner = "<div id='spinner'><span class='fa fa-4x fa-spin fa-spinner'></span></div>";
		$(self.element).append(spinner); // #blog-results
	}

	this.hideBlogSpinner = function(){
		$(self.element).children('#spinner').remove();
	}

	this.setLoading = function(trueOrFalse){
		if(trueOrFalse){
			self.showBlogSpinner();
		}else{
			self.hideBlogSpinner();
		}
		self.loading = trueOrFalse;
		return this;
	};
	...
	this.getBlog = function(){
		if(!self.loading)
		{
			...
			var ajaxData 		= {};
				ajaxData.key 	= self.APIKey;
				ajaxData.blogID = self.blogID;

			if(self.nextPageToken != ''){
				ajaxData.pageToken = self.nextPageToken;		
			}
			
			$.ajax({
				//url : 'https://www.googleapis.com/blogger/v3/blogs/'+self.blogID+'/posts/',
				url : 'php/blog.php',
				dataType : 'json',
				data : ajaxData
			})
	...
	this.listBlog = function(obj){
		// set next page token 
		self.nextPageToken = obj.nextPageToken;

		// display blog posts
		for (var i=0; i<obj.items.length;i++){
			var postHTML = self.postTemplate;
				postHTML = self.replacePostTemplate(obj.items[i], postHTML);
				//document.getElementById('blog-results').insertAdjacentHTML('beforeend', postHTML);
	...
	this.replacePostTemplate = function(obj, str){
		var returnThis = str;
			returnThis = returnThis.replace(/{postID}/g, obj.id);
			returnThis = returnThis.replace(/{blogTitle}/g, obj.title);
			returnThis = returnThis.replace(/{blogAuthor}/g, obj.author.displayName);
			returnThis = returnThis.replace(/{blogPublishedDate}/g, self.formatDateTime(obj.published));
			returnThis = returnThis.replace(/{blogContent}/g, obj.content);
		return returnThis;
	}


	this.getComments = function(postID, url){
	...
	this.getBlogTemplate = function(){
		return this.blogTemplate;
	};

	this.getCommentTemplate = function(){
		return this.commentTemplate;
	}
	...
	this.formatDateTime = function(dateTime){
		var d = new Date(dateTime);
		var returnThis = self.monthAsString( self.getRealMonth( d.getMonth() ) ) + ' ' + d.getDate() + ', ' + d.getFullYear();
			returnThis += ' at ';
			returnThis += self.get12HourFormat(d.getHours()) + ':' + self.getRealMinutes(d.getMinutes()) + ' ' + self.getAMPM(d.getHours());
		return returnThis;
	};

	this.getRealMonth = function(m){
		return m+1;
	};

	this.monthAsString = function(m){
		if(m == 1){return 'January';}
		if(m == 2){return 'February';}
		if(m == 3){return 'March';}
		if(m == 4){return 'April';}
	...
	this.get12HourFormat = function(h){
		if(h == 0){
			return 12;
		}else if(h <= 12){
			return h
	...
	this.getRealMinutes = function(m){
		return m <= 9 ? '0'+m : m;
		// if(m <= 9){
		// 	return '0'+m;
		// }else{
	...
	this.scrollHandler = function(){
		var windowHeight = window.innerHeight;
		var windowTop = $('body').scrollTop();
		var containerTop = $(self.element).position().top; 	//var containerTop = $('#blog-results-container').position().top;
		var containerHeight = $(self.element).height(); 	//var containerHeight = $('#blog-results-container').height();
		var containerBottom = containerTop + containerHeight;
		if(windowTop >= containerBottom - windowHeight){
			self.getBlog();
		}
	...
	this._init = function(){
		document.addEventListener('scroll', self.scrollHandler, false);
		self.getBlog();
	};	

	//document.addEventListener('DOMContentLoaded', self._init, false);
